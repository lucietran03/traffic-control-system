...
The project's real-world grounding is **Vietnam, right-hand traffic**, as required by Article 10 of Vietnam's Law on Road Traffic Order and Safety (No. 36/2024/QH15). As a contextual example, the team refers to the documented signalised layout of the Mai Chí Thọ–Đồng Văn Cống intersection in the former District 2 area of Ho Chi Minh City (Nguyen, *Capacity Analysis of Signalised Intersections in Motorcycle Dependent Cities*, Fig. A-7), rather than the Melbourne example in the brief. This is a reference for realistic lane and signal context, not a claim that the supplied project topology or timing values were measured at that site. With right-hand traffic, the median-side lane of each direction is used for through and permissive-left movements, and the kerb-side lane is used for through and permissive-right movements.
...
**Distance and speed assumptions [ASSUM, Proposed — Awaiting Confirmation]:** rather than mapping onto a literal named real-world intersection, the network is parameterised with illustrative arterial distances: `I1–I3 = 350 m`, `I3–I5 = 400 m` (on `R1`); `I2–I4 = 320 m`, `I4–I6 = 380 m` (on `R2`). Arterial travel speed is assumed at 60 km/h (16.67 m/s), consistent with the current Vietnamese maximum for qualifying urban divided roads or one-way roads with at least two motor-vehicle lanes (Circular 38/2024/TT-BGTVT, art. 6, table 1). The brief states — in both the official specification and the lecturer's clarification — that distance should be measured or estimated to determine timing requirements; these tunable values are used to compute coordination offsets in Section 20 without being presented as surveyed measurements.
...
- **Outputs/commands:** `SET_MODE(PEAK_FIXED | OFF_PEAK_SENSOR)`, `SET_TIMING_PROFILE(params within validated bounds)`, `REQUEST_OVERRIDE(type, target, duration)` toward `Lx`; toward `RLx`, only high-level, validated requests such as acknowledging/clearing a reported fault once physical repair is confirmed (see the reconciliation note below).
...
- **Explicit limitations [REQ][CLARIF]:** cannot set an individual light to a colour; cannot actuate any railway equipment (boom gate, flasher, or train signal) directly; cannot force a command through a safety invariant a local controller has rejected; cannot open a currently-latched pedestrian clearance early.
- **Reconciling the brief's railway wording [REQ][CLARIF]:** the brief states operators "will occasionally send commands to the intersections, boom gate control and train approach signals system," while also stating Central never directly controls an intersection's lights, and the lecturer's clarification states only `RLx` may actuate railway equipment. These are not contradictory once read together: `C1`'s "command to boom gate control" means a high-level, validated command addressed to `RLx` — the controller that owns that equipment — exactly the same command-abstraction pattern already used for `Lx` (never a raw actuation instruction sent straight to the hardware). `RLx` independently validates and decides whether to act on it, and may reject/NACK it (Section 18).
...
| 2 | Advance/queue sensor **[HD]** | ~60–80 m upstream of the stop line, one on each of the two crossing-facing intersection approaches per railway crossing | `Lx` | `QUEUE_WARNING` | Picture two trip lines on an approach: line A at the stop line (sensor #1) and line B about 70 m farther upstream (sensor #2). Sensor #1 answers "is anyone waiting at all?" Sensor #2 answers "has the queue grown back to line B?" It feeds the congestion-suppression logic in Section 16, not ordinary phase extension. It is a binary presence trip, not a vehicle counter. A dedicated keyboard key. |
...
|---|---|---|---|---|---|---|
| `PEAK_FIXED` | Predictable, coordinable timing at high demand | Time-of-day or `SET_MODE` | Fixed 2-phase cycle, arterial-priority split; the ordinary approach presence sensors are monitored for status/fault purposes only, not for extension; the separate advance/queue sensor (Section 9, item 2) keeps monitoring for congestion independently of mode | `SET_TIMING_PROFILE`, `SET_MODE`, override | `SET_MODE` or time-of-day change | Normal (7) |
...
| `DEGRADED_LOCAL` | Continue safely with no Central | Heartbeat timeout from `C1` | Retain last-known valid *timing parameters*; mode itself (`PEAK_FIXED`/`OFF_PEAK_SENSOR`) is not frozen and continues to be self-selected by local clock, exactly as when `C1` is reachable | None (link is down) | Comms restored + local pushes full state to `C1` | Orthogonal (not in the hazard hierarchy — an availability state, not a hazard) |
...
`OFF_PEAK_SENSOR`: a phase only receives green when its approach(es) report demand or hold a latched pedestrian request; green extends in 4 s increments **[ASSUM]** up to a 40 s cap **[ASSUM]** while demand persists; with zero demand anywhere, the intersection rests on the arterial phase (default main-road priority) [REQ — "differ between R1-R5, teams must state and justify"]. A latched connector demand is served after no more than one opposing arterial service opportunity has completed, subject to the required yellow and all-red clearance intervals. Persistent arterial demand cannot repeatedly extend or immediately reselect the arterial phase while a connector request remains pending — a structural anti-starvation guarantee rather than a weighting algorithm.
...
- A button input stuck active is coalesced into one pending request and reported as a fault. A permanently silent button is not detectable without an independent health signal because it is indistinguishable from "not pressed" **[ASSUM]**.
...
**Accepted trade-off:** using a fixed timer instead of a confirmed-exit sensor means the design does not, today, detect the specific edge case of a train that becomes abnormally slow or stalled on the crossing. This is a deliberate, documented simplification for the Core design — not an oversight — and the reserved exit sensor (Section 9, item 5) is the identified path for closing that gap later if time permits. This trade-off was reviewed a second time and deliberately kept as-is (rather than promoted to Core/HD scope) to protect the schedule for the scope already committed elsewhere (Section 23); "fail-safe," as used throughout this document, refers specifically to the gate-confirmed-closed-before-`PROCEED` interlock (Invariant #5/#6) and the fail-to-red behaviour on every other described fault — it is not a claim that every conceivable railway failure mode, including a stalled train past its assumed occupancy window, is covered.
...
| Failure | Response |
|---|---|
| Gate fails to confirm `CLOSED` before the deadline while a train is approaching | `FAULT_SAFE` (crossing): `RLx` forces/holds the train signal at `STOP` immediately and, in parallel (not as a prerequisite or a follow-up step), reports the fault to `C1`; the toward-crossing vehicle movement is held red until manually cleared. `RLx` never waits for `C1` to acknowledge before forcing `STOP` (Section 18). |
| Train approach sensor stuck **active** (keeps reporting `TRAIN_APPROACHING` without ever clearing) | **Detectable.** If the signal remains asserted well beyond the derived lead-time-plus-occupancy budget (Section 14) with no train ever confirmed clear, `RLx` treats this as anomalous, raises a fault, and fails toward the safe direction — crossing held at `WARNING`/gates down until an operator clears the fault. Mirrors the real-world principle that level-crossing equipment fails toward stopping road traffic, never toward allowing a train through unconfirmed. |
| Train approach sensor goes **silent** (stops signalling entirely, "failed dead") | **Accepted limitation — not detectable by the Core design.** A sensor that has permanently stopped signalling looks, from `RLx`'s point of view, identical to "no train is currently approaching" — the Core sensor design has no independent heartbeat/self-test signal to tell these two situations apart. This means a fully dead approach sensor silently removes that crossing's proactive train-warning capability — the *fail-open* direction, not fail-safe — and the design does not claim to detect it. This is stated plainly rather than implied away; the identified fix (a periodic self-test/heartbeat line on the sensor, independent of its detection line) is out of Core scope (Section 26). |
...
| Vehicle detector or pedestrian button stuck active | Detectable and reported locally. Vehicle detection is treated as continuous demand; repeated button activation is coalesced into one pending pedestrian request. |
| Vehicle detector or pedestrian button permanently silent | Accepted limitation — not detectable without an independent heartbeat or self-test, because silence is indistinguishable from no vehicle or no button press. If an explicit diagnostic or injected fault reports the failure, the affected demand input fails to the asserted state. |
...
`C1` monitors all 9 local controllers' state, timing profile, faults, and railway status (`C1` does not monitor itself, so the count is 9, not the network's total of 10 logical controllers), and issues three command classes: `SET_MODE`, `SET_TIMING_PROFILE`, `REQUEST_OVERRIDE` [REQ/CLARIF — "command abstraction," never a raw light colour]. Every command receives an explicit ACK/NACK. A NACK means the local controller judged the command unsafe (e.g. an override request during active railway pre-emption at that intersection) and it is surfaced to the operator as a rejected/faulted command, never silently dropped.
...
**Railway fault reporting is never a round trip through `C1` [TEAM].** When `RLx` detects a railway-equipment fault (Section 17), forcing the train signal to `STOP` and reporting the fault to `C1` are two **independent, parallel** actions — `RLx` never waits for a `C1` acknowledgement before forcing `STOP`. This matches the lecturer's description of the sequence (train gets a stop signal as soon as the issue exists; the error is *then* reported to the control room) and preserves local autonomy (Section 19) for the single most safety-critical decision in the system. `C1`'s only role after a fault is to display it and, once physical repair is confirmed, send `RLx` a validated fault-clear request (Section 8.1) — which `RLx` may still reject if the underlying condition has not actually cleared.
...
- Causes the controller to enter `DEGRADED_LOCAL`: it **retains its last known valid timing parameters** (green/yellow/all-red durations, extension caps, coordination offset) rather than reverting to an arbitrary hard-coded default — but mode selection itself is **not** frozen at whatever mode was active at disconnection. From that point on, the controller's own time-of-day clock becomes the sole authority for switching `PEAK_FIXED ↔ OFF_PEAK_SENSOR`, exactly as it would with `C1` reachable, using the last validated timing profile for whichever mode is currently selected.
...
| 2 | Arterial speed 60 km/h; illustrative per-segment distances `I1–I3=350m`, `I3–I5=400m`, `I2–I4=320m`, `I4–I6=380m` | The speed is consistent with the current Vietnamese maximum for qualifying urban divided roads or one-way roads with at least two motor-vehicle lanes (Circular 38/2024/TT-BGTVT). The brief and lecturer clarification require distance or travel time to inform coordination, so concrete tunable values are used rather than an abstract formula alone. | Feeds the coordination offsets computed in Section 20 (21 s, 24 s, 19 s, and 23 s) | No |
...
| 15 | Demo topology remains generic and parameterised; the documented Mai Chí Thọ–Đồng Văn Cống intersection is used only as a real-world contextual reference | Satisfies the brief's intent without claiming that the supplied topology or illustrative timing values were surveyed at that site | Preserves the supplied topology while making the real-world reference traceable | No, by design |
...
| 17 | A train approach sensor that goes permanently silent ("failed dead") is **not detectable** by the Core design — only a stuck-*active* sensor is detectable (Section 17) | The Core sensor design has no independent heartbeat/self-test signal separate from its detection line, so a dead sensor is indistinguishable from "no train currently approaching"; claiming otherwise would overstate what a single, simple sensor can report | A dead approach sensor silently removes that crossing's proactive warning capability (fail-*open*, not fail-safe) — an explicitly accepted residual risk, not something the system claims to solve; a dedicated periodic self-test/heartbeat line is the identified (out-of-Core) fix | No — internal scope clarification |
