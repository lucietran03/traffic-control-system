...
int main(int argc, char *argv[]) {
    // Task 2B: Client Code
    printf("Welcome to the Task 2B Client!\n");
	printf("Task 2B: Client running\n");

	int serverPID = 0;	
	int	serverCHID = 0;			

    // Automatically read connection data from the file
    FILE *fp = fopen(INFO_FILE, "r");
    if (fp != NULL) {
        fscanf(fp, "%d\n%d", &serverPID, &serverCHID);
        fclose(fp);
        printf("Successfully read PID (%d) and CHID (%d) from %s\n", serverPID, serverCHID, INFO_FILE);
    } else {
