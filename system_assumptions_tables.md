...
| PA-11 | A `CLEAR_ROUTE` override has a finite requested duration capped at **5 minutes**. It expires automatically unless a valid renewal is accepted and may be cancelled earlier by the operator. Every initial request and renewal remains subject to local safety validation. | Bounding and automatically expiring an override prevents a forgotten command from indefinitely displacing normal operation, while revalidation ensures that changed pedestrian, railway, or fault conditions retain priority. | `Lx` validates the requested duration and current safety conditions before accepting an override or renewal. A valid request receives `ACK`; an invalid or unsafe request receives `NACK` and does not extend the current expiry time. |
| PA-12 | Each local controller returns `ACK` or `NACK` within **1 s** of receiving a Central command. If execution must wait for a protected interval or a safe phase boundary, the controller returns `ACK_ACCEPTED_PENDING`; only activation is deferred, and the request is revalidated before activation. | The brief requires a Central-to-local command exchange but specifies no response deadline. A 1 s limit gives the operator timely feedback without allowing a Central command to bypass local safety sequencing. | Command acknowledgement and command activation are treated separately; meeting the 1 s response deadline never requires immediate execution. |
