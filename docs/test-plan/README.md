...
|---|---|---|---|---|---|
| 1 | [01-usecase-functional.md](01-usecase-functional.md) | UC-01..10 (usecase.md) — hành vi đầu-cuối theo từng use case | Functional / black-box | Bàn phím (`lx_sensor.c`/`rlx_sensor.c`/`c_operator.c`) + quan sát console/log | 45 |
| 2 | [02-state-machine-transition.md](02-state-machine-transition.md) | SC-01A/B/C, SC-02, SC-03A/B, SC-04A/B, SC-05 (STATE_CHARTS.md) — từng transition | State-machine transition | Bàn phím + quan sát trạng thái qua console/`c_hmi`; 1 số case cần chờ đúng mốc thời gian | 43 |
| 3 | [03-protocol-contract.md](03-protocol-contract.md) | 10 verb trong `ipc_msg.h` — mọi outcome ACK/ACK_PENDING/NACK(reason)/ERROR | Protocol / contract | Phần lớn qua `c_operator.c` (bàn phím); 1 số case sai định dạng cần **tool `test_client` riêng (chưa xây)** | 48 |
| 4 | [04-timing-assumptions.md](04-timing-assumptions.md) | Giá trị số/thời gian trong `system_assumptions_tables.md` (TC/TL/RC/PA/DP/CC) | Timing / performance | Đồng hồ bấm giờ tay hoặc đọc timestamp trong `central_log.txt` (không có đồng hồ chung, hệ thống dùng tick 100ms) | 32 |
| 5 | [05-fault-safety.md](05-fault-safety.md) | Watchdog, FAULT_SAFE, RC-06/09/10 gate fault | Fault-injection / safety | Bàn phím (phím `x`/`r`/`f` demo); 1 số case cần **debugger (gdb)** để mô phỏng treo thread hoặc gọi hàm trực tiếp không có UI | 24 |
| 6 | [06-concurrency-race.md](06-concurrency-race.md) | Race condition, input dồn dập, regression cho bug đã sửa | Concurrency / race | Bàn phím thao tác cực nhanh (nên script hóa nếu có thể), **lặp lại nhiều lần (N≥5-10)** để tăng khả năng bắt lỗi | 21 |
...
|---|---|---|---|
| 1 | ~~`RESULT_ACK` cho `MSG_REQUEST_FAULT_CLEAR` không thể tái hiện qua bàn phím — không có đường nào gọi `rlx_gate_command_open()` khi đang ở state `RLX_FAULT`, nên nhánh "fault clear thành công" chỉ verify được bằng debugger~~ — **ĐÃ SỬA**: `rlx_sensor.c`'s phím `r` gọi `rlx_gate_force_confirmed_open()` (`rlx_gate.c`) để ép gate vào trạng thái confirmed-open ngay trong khi RLx đang `RLX_FAULT`, sau đó `MSG_REQUEST_FAULT_CLEAR` qua Central (phím `f` của C1) tạo ra `RESULT_ACK` thật — toàn bộ nhánh nay test được thuần bàn phím, không cần debugger | Đã đóng | Xem test case mới nhất trong `05-fault-safety.md` dùng phím `r` (`TC-FAULT-22`) |
...
| 3 | ~~DP-02 (tự động chuyển Peak/Off-Peak theo giờ) chưa nối vào runtime — `c_mode_eng_select_mode()` không được gọi với giờ thật ở đâu cả, chỉ đổi mode bằng tay qua operator~~ — **ĐÃ SỬA**: `c_main.c`'s `on_pulse()` nay gọi `c_mode_eng_auto_check()` mỗi tick 1Hz với giờ thật (hoặc giờ demo bị ép qua phím `d`/`a` của `c_operator.c`) và broadcast `SET_MODE` cho mọi Lx khi qua mốc lịch trình | Đã đóng | Test bằng phím `d` (ép giờ demo) / `a` (hủy ép) của `c_operator.c` để buộc qua mốc 06:00/09:00 mà không cần chờ đồng hồ thật |
