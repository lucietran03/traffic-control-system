...
- **Tick pha đèn/railway cố định 100 ms** (`LX_PHASE_TICK_MS`, `lx_timer.h:53`) cho `Lx`, và tick 1000 ms cho `RLx`/heartbeat/watchdog. Mọi ngưỡng thời gian chỉ được cập nhật tại các bội số của tick đó — không có ý nghĩa đòi hỏi độ chính xác dưới 100 ms (Lx) hoặc dưới 1 s (RLx/PA-07).
...
| `LX_PHASE_TICK_MS` | 100 | `lx_timer.h:53` | (tick hệ thống) |
| `LX_WALK_MS` | 6000 | `lx_timer.h:70` | TL-05 (placeholder, không phải giá trị spec) |
| `LX_FLASHING_DONT_WALK_MS` | 4000 | `lx_timer.h:71` | TL-05 (placeholder) |
| `LX_DRAIN_MAX_EXTENSION_MS` | 60000 | `lx_timer.h:84` | CC-03 |
| `LX_CYCLE_LENGTH_MS` | 90000 (= 48+4+2+30+4+2 s, tính từ các hằng số trên) | `lx_timer.h:94-95` | TL-02/TC-02 |
...
`c_mode_eng_select_mode()` (`app/central/src/c_mode_eng.c:63-69`) vẫn là một
hàm thuần nhận `current_hour` làm tham số do **caller truyền vào** — bản
thân hàm không tự đọc đồng hồ hệ thống. Tuy nhiên, `c_main.c`'s `on_pulse()`
(nhánh `IPC_PULSE_HEARTBEAT_TICK`, `app/central/src/c_main.c:149-220`)
chính là caller đó và chạy mỗi 1 Hz: nếu
`ctx->mode_eng.demo_hour_override_active` chưa được bật, nó gọi `time(NULL)`
rồi `localtime_r()` (`c_main.c:189-193`) để lấy giờ đồng hồ tường thật, sau
đó gọi `c_mode_eng_auto_check(&ctx->mode_eng, current_hour, &auto_mode)`
(`app/central/src/c_mode_eng.c:71-93`) và broadcast `SET_MODE` tới cả 6 `Lx`
qua `c_comm_broadcast_set_mode()` (`c_main.c:215-218`) bất cứ khi nào mode
theo lịch thực sự đổi so với lần kiểm tra trước, kèm dòng log
`"Auto peak-hour switch: hour=%u -> mode=%s, broadcasting to all Lx"`. Nói
cách khác: **việc tự động chuyển `PEAK_FIXED` ↔ `OFF_PEAK_SENSOR` theo giờ
đồng hồ (DP-02) ĐÃ được nối dây (wired) vào hệ thống đang chạy** — đây
không còn là một khoảng trống hiện thực. `c_operator.c` còn có phím `d`
(`handle_demo_hour()`, `c_operator.c:408-433`) để ép một giờ giả lập
(`mode_eng->demo_hour`/`demo_hour_override_active`) đứng thay cho "giờ hiện
tại", và phím `a` (`handle_resume_automatic()`, `c_operator.c:439-447`) để
quay lại dùng đồng hồ tường thật — cho phép tester ép hệ thống tới sát ranh
giới peak/off-peak mà không phải chờ giờ thật trôi qua. Các test case
DP-TIME-xx bên dưới phản ánh đúng hành vi đã-nối-dây này và dùng phím `d` để
tạo ranh giới có thể quan sát được trong một phiên test ngắn.
...
- **Các bước**:
  1. Trên console `L1`, chờ dòng log `"Lx 1: signal phase now ARTERIAL GREEN"` xuất hiện.
  2. Ngay khi thấy dòng log đó, bấm Start trên đồng hồ bấm giờ điện thoại.
  3. Chuyển sang/quan sát song song console `L3`, chờ dòng log `"Lx 3: signal phase now ARTERIAL GREEN"` của **chu kỳ ngay sau đó** (không phải một chu kỳ ngẫu nhiên khác).
...
- **Các bước**: Lặp lại thao tác TC-TIME-01 nhưng bấm Start tại `"Lx 3: signal phase now ARTERIAL GREEN"` và Stop tại `"Lx 5: signal phase now ARTERIAL GREEN"` của chu kỳ kế tiếp. Đồng thời đo lại trực tiếp L1 → L5 (bắc cầu 2 chu kỳ chờ nếu cần) để đối chiếu cộng dồn.
...
- **Các bước**: Đo `"Lx 2: signal phase now ARTERIAL GREEN"` → `"Lx 4: signal phase now ARTERIAL GREEN"` (kỳ vọng ~19 s) và `"Lx 4: ..."` → `"Lx 6: ..."` (kỳ vọng ~23 s), cùng phương pháp stopwatch như trên.
...
- **Các bước**:
  1. Ghi lại thời điểm gửi lệnh (theo đồng hồ bấm giờ) và thời điểm `"Lx 1: signal phase now ARTERIAL YELLOW"` xuất hiện ngay sau đó (kết thúc pha ARTERIAL_GREEN hiện tại).
...
- **Môi trường**: (D) — cần công cụ `test_client` (chưa có trong repo, xem `docs/test-plan/03-protocol-contract.md` mục "Quy ước môi trường (A/B/C/D)" và "Đề xuất công cụ test_client"). `C1` operator console (`handle_timing_profile()`, `app/central/src/c_operator.c`) **không** phải một cách hợp lệ để tạo case này: hàm này chỉ hỏi đúng một số nguyên `1` hoặc `2` để chọn chuỗi cố định `R1`/`R2` (`c_mode_eng_get_chain()`, offset lấy từ `R1_L1_OFFSET_MS=0`/`R1_L3_OFFSET_MS=21000`/`R1_L5_OFFSET_MS=45000`/`R2_L2_OFFSET_MS=0`/`R2_L4_OFFSET_MS=19000`/`R2_L6_OFFSET_MS=42000` trong `app/central/includes/c_mode_eng.h`) — không có đường nào để nhập trực tiếp một `offset_ms` tuỳ ý như 89999 hay 90000 qua console này. Chỉ một message dây trực tiếp từ `test_client` mới gửi được `offset_ms` tuỳ ý.
- **Chuẩn bị**: `L1` đang chạy bình thường, không có fault.
- **Các bước**:
  1. `test_client` gửi trực tiếp tới `L1` `ipc_request_t{verb=MSG_SET_TIMING_PROFILE, payload.timing_profile={offset_ms=89999}}` → quan sát log `C1`: `"C1: SET_TIMING_PROFILE to 1 -> ACK"`.
  2. `test_client` gửi `offset_ms = 90000` (đúng bằng `LX_CYCLE_LENGTH_MS`) → quan sát log `C1`.
...
- **Tham chiếu chéo**: Case này trùng ranh giới với `docs/test-plan/03-protocol-contract.md`'s TC-MSG-3 (`offset_ms=90000` → NACK) và TC-MSG-4 (`offset_ms=89999` → ACK), mục 1 "MSG_SET_TIMING_PROFILE" — cùng môi trường (D), cùng cơ chế test_client.
...
- **Các bước**: Bấm Start ngay khi `"Lx 1: signal phase now ARTERIAL GREEN"` xuất hiện, không tạo bất kỳ demand nào trong suốt phiên. Bấm Stop khi `"Lx 1: signal phase now ARTERIAL YELLOW"` xuất hiện.
...
- **Các bước**: Bấm Start tại `"signal phase now ARTERIAL YELLOW"` (hoặc CONNECTOR YELLOW), Stop tại `"signal phase now ALL RED (...)"` kế tiếp.
...
- **Môi trường**: (A)
- **Chuẩn bị**: Như trên.
- **Các bước**: Bấm Start tại `"signal phase now ALL RED (A to B)"`, Stop tại `"signal phase now CONNECTOR GREEN"` (hoặc pha kế tiếp tương ứng).
...
- **Loại**: Positive
- **Liên quan**: TL-05/TL-06, `LX_WALK_MS = 6000`, `LX_FLASHING_DONT_WALK_MS = 4000` (`lx_timer.h:70-71` — **lưu ý đây là giá trị placeholder tự chọn của nhóm, không phải số bắt buộc từ đề bài**, nhưng vẫn phải khớp đúng con số đã code).
- **Môi trường**: (A)
...
- **Liên quan**: TL-02, `LX_CYCLE_LENGTH_MS = 90000` (`lx_timer.h:94-95`, tính từ 48+4+2+30+4+2)
- **Môi trường**: (A)
...
- **Các bước**: Bấm Start tại một lần `"signal phase now ARTERIAL GREEN"` bất kỳ, Stop tại lần `"signal phase now ARTERIAL GREEN"` **kế tiếp** (đúng 1 chu kỳ đầy đủ: arterial green+yellow+all-red+connector green+yellow+all-red).
...
> **Lưu ý trước khi test**: như đã nêu ở mục 0.3, việc tự động chuyển mode
> theo giờ đồng hồ **đã được nối vào runtime** — `c_main.c`'s `on_pulse()`
> đọc giờ đồng hồ tường thật (hoặc giờ giả lập ép bằng phím `d`) mỗi 1 Hz
> và gọi `c_mode_eng_auto_check()`, broadcast `SET_MODE` tới toàn bộ 6 `Lx`
> khi mode theo lịch đổi. Ba test case dưới đây khai thác đúng cơ chế đó,
> dùng phím `d`/`a` của `c_operator.c` để ép ranh giới peak/off-peak xảy ra
> trong một phiên test ngắn thay vì phải chờ giờ thật trôi qua.
...
### DP-TIME-02: Edge case — ép giờ giả lập qua ranh giới 09:00 bằng phím `d`, xác nhận broadcast SET_MODE tới cả 6 Lx (và log tự-động-phát hiện khi dùng đồng hồ tường thật)
- **Loại**: Edge case (ranh giới giờ) + Positive (xác nhận cơ chế auto-switch DP-02 đã nối dây, hoạt động đúng)
- **Liên quan**: DP-02; `c_main.c`'s `on_pulse()` (`app/central/src/c_main.c:149-220`) gọi `c_mode_eng_auto_check()` (`c_mode_eng.c:71-93`) mỗi 1 Hz; `c_operator.c`'s `handle_demo_hour()`/`handle_resume_automatic()` (`c_operator.c:408-433`/`439-447`, phím `d`/`a`).
- **Môi trường**: (B) hoặc (C) — `C1` + đủ 6 `Lx` (để xác nhận toàn bộ 6 controller đều nhận `SET_MODE`, không chỉ một số).
- **Chuẩn bị**: Khởi động `c_main`, `lx_main 1..6`. Trên console `C1`, gõ `d`, nhập giờ `8` (còn trong khoảng peak 06-09) để đưa hệ thống về một baseline biết trước là `PEAK_FIXED`.
- **Các bước**:
  1. **Nhánh xác nhận broadcast (nhanh, tất định)**: Gõ `d` lần nữa, nhập giờ `9` (đúng bằng `C_MODE_ENG_DEFAULT_PEAK_END_HOUR`, ranh giới sang `OFF_PEAK_SENSOR`). Quan sát `central_log.txt` ngay lập tức: dòng `"Operator: demo hour forced to 9 -> schedule implies mode=OFF_PEAK_SENSOR, broadcasting to all Lx"`, theo sau là 6 dòng `"C1: SET_MODE to <id> -> ACK"` (id = 1..6). Xác nhận qua HMI/log từng `Lx` rằng `operating_mode` đã đổi sang `OFF_PEAK_SENSOR` ở cả 6 controller.
  2. **Nhánh xác nhận đúng đường tự động của `on_pulse()` (dùng đồng hồ tường thật)**: Gõ `a` để tắt demo override (log `"Operator: resuming automatic (real clock) peak-hour switching"`). Vì `handle_demo_hour()` tự đồng bộ `last_auto_mode` trước khi trả về — đúng để tick 1 Hz kế tiếp không bắn trùng broadcast — dòng log riêng của `on_pulse()`, `"Auto peak-hour switch: hour=%u -> mode=%s, broadcasting to all Lx"` (`c_main.c:215-218`), **chỉ** xuất hiện khi chính `on_pulse()` tự phát hiện đổi mode từ giờ đồng hồ tường thật, không phải qua phím `d`. Để quan sát dòng log này trong một phiên test ngắn (thay vì chờ giờ thật trôi tới ranh giới), chỉnh đồng hồ hệ thống của VM chạy `C1` (`date`, quyền root, chỉ trên VM test) tới ngay trước 09:00:00 rồi để trôi qua mốc đó.
- **Kết quả mong đợi**: Bước 1 xác nhận cơ chế broadcast-tới-6-Lx của DP-02 hoạt động đúng qua đường `d` (tất định, nhanh, tái sử dụng cùng `c_mode_eng_select_mode()`/`c_comm_broadcast_set_mode()` mà `on_pulse()` dùng). Bước 2 xác nhận thêm rằng khi KHÔNG có demo override, `on_pulse()` tự phát hiện ranh giới giờ thật và tự bắn broadcast + log `"Auto peak-hour switch: ..."` mà không cần bất kỳ thao tác operator nào tại đúng thời điểm ranh giới. Cả hai kết quả PASS cùng xác nhận: DP-02 **đã được nối dây (wired)** vào hệ thống đang chạy — khác với kết luận "chưa nối dây" của phiên bản tài liệu trước đây.

### DP-TIME-03: Positive — đường thay thế: SET_MODE thủ công qua operator console
- **Loại**: Positive
...
- `"RLx: flashers ON (train approaching, direction %u)"` — bắt đầu WARNING (`rlx_signal.c:24`), tương ứng T0 của `TRAIN_APPROACHING`.
...
- `"RLx: flashers OFF (gates confirmed open)"` — xác nhận OPEN (`rlx_signal.c:29`).
- `"RLx: FAULT latched (GATE_CONFIRM_MISSING, bit 0x...) ..."` — báo fault (`rlx_signal.c:49`).
...
- **Kết quả mong đợi**: KHÔNG có dòng `"FAULT latched"` trước **19.5 s**; dòng `"FAULT latched (GATE_CONFIRM_MISSING, bit ...) ... commanding gates DOWN"` PHẢI xuất hiện trong cửa sổ **20.0–21.0 s** kể từ lúc `flashers ON` (dung sai nới hơn vì cộng 2 tick 1000ms rời rạc + sai số tay).
...
**Tổng: 32 test case**, bao phủ toàn bộ các nhóm assumption thời gian được yêu cầu (TC-01..05, TL-01..06, DP-01/02, CC-01/03, RC-03/04/06, PA-07, PA-11/12), mỗi giá trị số có ít nhất một test positive và một test edge/boundary tại ranh giới tick, cùng các test negative cho các ngưỡng NACK theo PA-09/PA-11.
