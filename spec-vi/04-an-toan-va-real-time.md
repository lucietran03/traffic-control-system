...
| Lỗi | Phản ứng |
|---|---|
| Gate không xác nhận `CLOSED` trước deadline trong khi có tàu đang tới | `FAULT_SAFE` (crossing): train signal bị ép/giữ `STOP`, báo lỗi ngay cho `C1`, hướng đi về crossing bị giữ đỏ cho tới khi được xoá lỗi thủ công |
| Sensor tàu lỗi (không có tín hiệu / bị kẹt) | Fail-safe về "có thể có tàu" — gate giữ nguyên xuống / không mở lại cho tới khi operator xoá lỗi. Phản ánh nguyên tắc thực tế: thiết bị chắn đường sắt luôn fail theo hướng chặn xe, không bao giờ fail theo hướng cho tàu qua khi chưa xác nhận |
...
- Khiến controller vào `DEGRADED_LOCAL`: tiếp tục theo mode/timing profile hợp lệ gần nhất thay vì quay về 1 mặc định cứng tuỳ tiện, và tự chọn Peak/Off-Peak theo đồng hồ local.
