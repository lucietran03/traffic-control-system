...
| Sensor xe hoặc nút người đi bộ bị kẹt active | Phát hiện và báo lỗi cục bộ. Sensor xe được xử lý như nhu cầu liên tục; tín hiệu nút lặp lại được gộp thành 1 yêu cầu người đi bộ đang chờ. |
| Sensor xe hoặc nút người đi bộ im lặng vĩnh viễn | Giới hạn được chấp nhận — không thể phát hiện nếu không có heartbeat/self-test độc lập, vì im lặng không thể phân biệt với không có xe hoặc không ai nhấn nút. Nếu lỗi được báo bởi diagnostic rõ ràng hoặc fault injection, đầu vào nhu cầu tương ứng fail về trạng thái asserted. |
...
`C1` giám sát trạng thái, timing profile, lỗi và trạng thái đường sắt của cả **9 local controller** (`L1-L6` + `RL1-RL3` — `C1` không tự giám sát chính nó, nên là 9, không phải tổng 10 logical controller của toàn mạng lưới), và phát ra 3 nhóm lệnh: `SET_MODE`, `SET_TIMING_PROFILE`, `REQUEST_OVERRIDE` [REQ/CLARIF — "command abstraction," không bao giờ là 1 màu đèn thô]. Mỗi lệnh nhận về 1 trong các kết quả: `ACK` (chấp nhận, áp dụng ngay), `ACK_ACCEPTED_PENDING` (chấp nhận nhưng phải chờ qua 1 khoảng bảo vệ hoặc ranh giới pha an toàn mới được áp dụng — ví dụ `REQUEST_OVERRIDE` tới trong lúc pedestrian clearance đang chạy), `NACK` (từ chối vì lý do an toàn, kèm lý do cụ thể), hoặc `ERROR` (không xử lý được, ví dụ lệnh sai định dạng — khác `NACK` ở chỗ `NACK` là lệnh hợp lệ nhưng bị đánh giá không an toàn, còn `ERROR` là bản thân lệnh không xử lý được). Local controller phải trả lời (`ACK`/`ACK_ACCEPTED_PENDING`/`NACK`/`ERROR`) trong vòng **1 giây** kể từ khi nhận lệnh — kể cả khi kết quả là `ACK_ACCEPTED_PENDING`, việc *xác nhận đã nhận lệnh* không bao giờ chờ tới lúc lệnh thực sự được áp dụng; yêu cầu bị hoãn sẽ được xác thực lại ngay trước khi kích hoạt **[ASSUM, PA-12]**. `NACK` nghĩa là local controller đánh giá lệnh đó không an toàn (ví dụ 1 yêu cầu override trong lúc railway pre-emption đang diễn ra tại giao lộ đó) và điều này được hiển thị cho operator như 1 lệnh bị từ chối/báo lỗi, không bao giờ bị âm thầm bỏ qua.
...
| Lệnh Central → `ACK`/`ACK_ACCEPTED_PENDING`/`NACK`/`ERROR` (Mục 18, PA-12) | `SET_MODE`/`SET_TIMING_PROFILE`/override | Soft/firm | < 1 giây — deadline này là để **xác nhận đã nhận lệnh**, không phải để lệnh được áp dụng xong (`ACK_ACCEPTED_PENDING` vẫn tính là trả lời đúng hạn) | 2 lần retry, sau đó `C1` đánh dấu controller không liên lạc được/báo động liên lạc |
